...
function createUI(){
    ...
    if(isIDE){if(isMainWindow) tryCreateIDE();
    ...
    if(st.bar){st.bar.onclick=function(e){e.stopPropagation();if(st.popup)st.popup.style.display=st.popup.style.display==='none'?'block':'none';};}
